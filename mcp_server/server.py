...
def _validate_accept_header(accept_header: Optional[str]) -> bool:
    ...
    accept_types = [header.strip().lower() for header in accept_header.split(",")]
    has_json = any("application/json" in _type for _type in accept_types)
    has_sse = any("text/event-stream" in _type for _type in accept_types)
...
async def handle_mcp_request(
    ...
    if request.method == "initialize":
        ...
        if not session.ready_for_operation:
            error_response = MCPResponse(
                id="server-error",
                error=ErrorResponse(code=-32600, message="Session in not ready"),
            )
            return Response(
                status_code=400,
                content=error_response.model_dump_json(),
